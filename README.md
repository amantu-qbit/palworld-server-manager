...
Pick a player from the roster to see their party, Pal box, and (with the Bridge stationed at a base) their base Pals — each Pal card shows level, gender, condenser stars, element, and HP/ATK/DEF IVs. The **Character** tab shows the player's own stats, status points, and unlocked technology tree; the **Inventory** tab renders their carried items as a game‑style grid. With editing enabled you can adjust levels, souls, talents, skills, and more — or heal, clone, and delete Pals.
